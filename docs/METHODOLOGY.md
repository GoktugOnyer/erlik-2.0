# Thesis Update Guide + Chapter 3: Methodology

---

## THESIS UPDATE GUIDE (for AI integration)

This document contains the complete methodology chapter plus a summary of ALL changes that need to be integrated into the thesis. The current thesis draft only covers cold and warm session types. The following concepts are NEW and must be added throughout the thesis (Introduction, Methodology, Results, Discussion):

### THESIS STRUCTURE (current as of 6-04.docx)

The thesis has these chapters:
1. Introduction (Background, RQs, Contributions, Scope)
2. State of the Art (LLMs in Cybersecurity, Tool-Use LLMs, Automated Pentest Systems, MCP, Benchmarks, Gap Analysis)
3. Requirements and System Analysis (Problem Def, Stakeholders, FR/NFR, Use Cases)
4. Methodology (Research Design, Testbed, Dashboard, MCP Architecture, Safety Gate, Fault Tolerance, Context Management, Report Gen, Model Hosting/Fine-Tuning, Dataset Engineering, Evaluation Metrics, Ablation Design, Ethics, Reproducibility, Threats to Validity)
5. Results (all POST-EXAMINATION placeholders)
6. Discussion (all POST-EXAMINATION placeholders)
7. Conclusion (mostly POST-EXAMINATION placeholders)

### WHAT THE THESIS ALREADY COVERS WELL
- MCP architecture justification (single coarse-grained server) — well explained
- Toolset presets (Core-10/Standard-20/Full-30) with rationale — already present
- RQ1-RQ4 + RQ3-b definitions with hypotheses — thorough
- Deterministic capability helpers (login-helper, diff-view, interactive-pw) — well explained
- Safety gate 3-tier command classification — covered
- Fault tolerance (model output validation, tool failure handling, loop detection) — covered
- LoRA/QLoRA fine-tuning methodology — detailed procedure
- State of the Art / Related Work — 44 references, comparative analysis
- Cold vs Warm start explanation — covered in RQ4

### WHAT THE THESIS IS MISSING OR NEEDS UPDATING

### NEW CONCEPTS NOT IN CURRENT THESIS

**1. Chain Sessions (4-Phase Sequential Testing) — MAJOR ADDITION, NOT IN THESIS AT ALL**

The thesis currently only describes two session types in RQ4: cold start and warm start. A third session type — chain sessions — has been fully implemented and evaluated but is NOWHERE in the thesis. This is the single biggest gap.

Chain sessions execute four sessions in sequence, each dedicated to a specific penetration testing phase:
1. Recon — port scanning, service identification, technology fingerprinting
2. Discovery — directory enumeration, API endpoint discovery, parameter finding
3. Vulnerability Scanning — injection testing, XSS probing, misconfiguration scanning
4. Exploitation — authentication attacks, authorisation bypass, business logic testing

Each phase receives the full turn budget (e.g., 30 turns per phase = 120 total turns across the chain). Context from each completed phase is compiled via `_compile_chain_context()` and passed to the next phase via the system prompt. The chain imposes expert methodology structure on the agent rather than letting it explore freely. Auto-progression handles phase transitions automatically.

Where to add in thesis:
- RQ4 definition (Section 1.2): Add chain as third condition alongside cold and warm. Update H4 to include chain hypothesis.
- Section 4 Methodology: Add full chain description (see Section 3.5.3 of this document)
- Section 5 Results: Add chain vs cold vs warm comparison
- Section 6 Discussion: Chain architecture as key finding

**2. Toolset Tiers — ALREADY IN THESIS (Section 4.4.2)**

The thesis already covers toolset presets well in Section 4.4.2 "Toolset Presets as a First-Class Experimental Dimension" and RQ3-b. No major addition needed. The methodology document below provides additional detail on per-tool OWASP coverage mapping that could supplement the existing description.

**3. Ground Truth Validation (41 Juice Shop Vulnerabilities) — PARTIALLY IN THESIS**

The thesis mentions "30-50 benchmark cases" and "per-vulnerability-category analysis" but does NOT describe the actual 41-entry ground truth catalogue, the scoring-based matching algorithm (score >= 2 threshold with type+URL+param+evidence dimensions), or the programmatic auto-detection system. These are implemented but not documented in the thesis.

Where to add:
- Section 4 Methodology: Add ground truth catalogue description, matching algorithm, auto-detection explanation (see Section 3.6 of this document)
- Section 5 Results: Report TP/FP/precision/coverage alongside raw findings

**4. Juice Shop State Reset Between Sessions — NOT IN THESIS**

The thesis does not mention resetting Juice Shop between sessions. This is a critical methodological control that must be documented. Each session now starts with a freshly restarted Juice Shop (in-memory SQLite resets on restart), eliminating cross-session state pollution.

Where to add:
- Section 4 Methodology: Add to testbed description (see Section 3.6.4 and 3.8.6 of this document)

**5. Removal of Wall-Clock Ceilings — NOT IN THESIS**

The thesis mentions tool-specific timeouts (Section 4.6.2) but does not discuss session-level wall-clock ceilings or their removal. The final design uses turn count as the sole termination criterion to ensure fair comparison across model sizes.

Where to add:
- Section 4 Methodology: Add as experimental control (see Section 3.7.3 of this document)

**6. Programmatic Finding Detection (_auto_detect_findings) — NOT IN THESIS**

The thesis describes tool output parsing but does NOT explain that vulnerability detection is programmatic (regex-based) rather than LLM-dependent. This is a critical design decision for scientific fairness — without it, larger models might score higher simply because they articulate findings better in JSON.

Where to add:
- Section 4 Methodology: Add as key design decision (see Section 3.7.4 of this document)

**7. Sprint Matrix Evaluation Design — NOT IN THESIS**

The thesis describes ablation design abstractly (Section 4.12) but does NOT describe the actual sprint matrix runner (sprint_matrix.py), the 27-session factorial execution, the per-session metrics collection, or the repeat-run variance estimation. The actual experimental execution procedure is missing.

Where to add:
- Section 4 Methodology: Add full execution procedure (see Section 3.8 of this document)

**8. Turn Count as Scaling Parameter — NOT IN THESIS**

The thesis does not mention turn counts (15, 30, 45) as an experimental dimension. The current thesis describes step limits (Section 4.6.3) but not their systematic variation.

Where to add:
- Section 4 Methodology: Add as independent variable

**9. Cloud Infrastructure Details — NEEDS UPDATING**

The thesis mentions "RunPod, NVIDIA A100 80 GB" (Section 4.8.1) but the actual evaluation runs on SimplePod with RTX 4090 (24 GB VRAM), 32 CPU cores, Ubuntu 22.04, using ERLIK_NATIVE mode (no Docker). This needs correction.

Where to add:
- Section 4 Methodology: Update infrastructure specification

**10. LoRA Fine-Tuning — ALREADY IN THESIS (Section 4.8), UPDATE SCOPE**

The thesis already has a detailed fine-tuning methodology. The plan is now to fine-tune ALL THREE model sizes (7B, 14B, 32B) using the same training data and LoRA config, producing a full 6-model comparison (3 baseline + 3 fine-tuned). This requires an A100 80GB GPU for the 32B fine-tuning. 7B and 14B can be fine-tuned on RTX 4090. Update Section 4.8 to reflect this expanded scope.

**11. DVWA (Tier 2 Evaluation) — IN THESIS BUT MAY BE DESCOPED**

The thesis describes a two-tier evaluation (Juice Shop + DVWA). If DVWA evaluation is not being conducted due to time constraints, this needs to be acknowledged as a limitation or future work.

**12. MCP Architecture — ALREADY IN THESIS (Section 4.4)**

Well covered. No changes needed.

**13. Qwen2.5-Instruct vs Qwen2.5-Coder Comparison — IN THESIS (RQ3/H3a) BUT MAY NOT BE RUN**

The thesis describes comparing code-specialised vs general-purpose models (Coder vs Instruct at 32B). If this comparison is not being run, update accordingly.

### RESEARCH QUESTIONS (verify these match your thesis)

| RQ | Question | Answered By |
|----|----------|-------------|
| RQ1 | Can LLMs autonomously orchestrate penetration testing tools to discover vulnerabilities? | All 81 conditions — if any session finds true positives via autonomous tool selection |
| RQ2 | How does the agentic framework structure (cold/warm/chain) affect testing effectiveness? | Session type comparison (cold vs warm vs chain) |
| RQ3 | Does model size affect penetration testing performance? | Model comparison (7B vs 14B vs 32B) |
| RQ3-b | Does action-space size (number of tools) affect small-model performance? | Toolset tier comparison (Core-10 vs Standard-20 vs Full-30) |
| RQ2 | Does domain-specific LoRA fine-tuning improve pentesting performance across model sizes? | 6-model comparison: 3 baseline (7B/14B/32B) + 3 fine-tuned (7B-ft/14B-ft/32B-ft), same dataset and LoRA config |

### FINDINGS DATA

Results data will be added once the clean evaluation runs complete. The new runs include:
- Juice Shop reset between every session (clean target state)
- No wall-clock ceilings (turn count is the sole termination criterion)
- Ground truth validation per session (true_positives, false_positives, precision, ground_truth_coverage)
- 3 repeat runs of representative config for statistical variance

Clean runs are currently in progress on the cloud (RTX 4090). Expected completion: 7B ~5h, 14B ~6h, 32B ~16h.

Once complete, the results section should include:
- Overall summary table (findings, TP, FP, precision, coverage per model)
- By session type (cold vs warm vs chain with chain multiplier)
- By toolset tier (Core-10 vs Standard-20 vs Full-30)
- By turn count (15 vs 30 vs 45 with diminishing returns analysis)
- Cold vs Chain delta table
- All individual session results with ground truth metrics
...
| **RQ2**: Does domain-specific LoRA fine-tuning improve pentesting performance? | LoRA fine-tuning experiment | 6-model comparison: 3 baseline + 3 fine-tuned. Tests whether specialisation helps at every scale and whether fine-tuned 7B can match baseline 32B. |
...
- **True positives (TP)**: findings validated against a ground truth catalogue of 41 known Juice Shop vulnerabilities
...
A catalogue of 41 known Juice Shop vulnerabilities serves as ground truth. Each entry was derived from Juice Shop's official challenge documentation and verified manually:

| OWASP Category | Severity | Count | Example Vulnerabilities |
|---------------|----------|-------|------------------------|
| A01: Broken Access Control | Critical-Medium | 7 | IDOR on basket/order APIs, admin panel bypass, user enumeration, forged feedback, product manipulation |
| A02: Cryptographic Failures | High-Medium | 4 | FTP directory with sensitive files, backup file exposure, unsalted MD5 password hashes, exposed source maps |
| A03: Injection (SQLi) | Critical-High | 3 | SQL injection on product search, login bypass via SQLi, blind SQLi on user endpoint |
| A03: Injection (XSS) | High-Medium | 5 | Reflected XSS on search, DOM XSS, stored XSS on feedback, XSS on track order, XSS via product review |
| A05: Security Misconfiguration | Medium-Info | 4 | robots.txt information leakage, Swagger/OpenAPI exposure, missing security headers, Prometheus metrics |
| A07: Auth Failures | Critical-Medium | 6 | No rate limiting on login, weak admin credentials, JWT weak secret, JWT none algorithm, weak password reset, brute-forceable security questions |
| Information Disclosure | Medium-Info | 3 | Verbose Express error pages with stack traces, server/technology header disclosure, exposed security.txt |
| Other (CORS, SSRF, etc.) | High-Medium | 9 | CORS wildcard origin, SSRF on profile image, open redirect, unrestricted file upload, XXE, prototype pollution, CRLF injection, command injection |
...
| Wall-clock ceilings | Removed (0 = unlimited) | Turn count is the fair comparison unit |
...
| Ground truth | 41 fixed entries | Consistent validation baseline |
...
### 3.7.3 Why Wall-Clock Ceilings Were Removed

Early runs used time ceilings (60 minutes for sessions, 120 minutes for chains) to prevent stuck runs from blocking the matrix. This introduced systematic bias: the 32B model's chains consistently hit the 120-minute ceiling (being 3-4x slower per turn than 7B), while 7B chains completed in 30 minutes. The 32B chains were forcibly stopped mid-execution, truncating their findings.

The final design removes all wall-clock ceilings. Turn count is the sole termination criterion. Each model gets the same number of reasoning-and-action cycles regardless of per-turn speed. The trade-off is longer runtime for larger models (32B matrices take approximately 16 hours vs. 5 hours for 7B).
...
| Ground truth | 41 entries, embedded in source + exported per run |
