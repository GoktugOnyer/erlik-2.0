...
async def resolve(db, presented: str | None) -> tuple[str | None, str | None]:
    """Map a presented token to (operator_id, name), or (None, None).
    ...
    cur = await db.execute(
        "SELECT id, name, token_hash, status FROM operators WHERE token_hash = ?",
        (token_hash(presented),),
    )
    row = await cur.fetchone()
    if not row:
        return None, None
    op_id, name, stored, status = row[0], row[1], row[2], row[3]
...
async def create(db, name: str, created_by: str | None = None) -> dict:
    ...
    await db.execute(
        "INSERT INTO operators (id, name, token_hash, status, created_by) "
        "VALUES (?, ?, ?, 'active', ?)",
        (op_id, name, token_hash(token), created_by),
    )
    await db.commit()
...
async def listing(db) -> list[dict]:
    ...
    cur = await db.execute(
        "SELECT o.id, o.name, o.status, o.created_at, o.last_seen_at, o.created_by, "
